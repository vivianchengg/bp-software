...
// Defining interfaces for modals 
interface Props {
  open: boolean,
  refresh: boolean,
...
// Regex to check the @email.com in inputted emails
const isValidEmail = (email: string): boolean => {
  const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  return emailPattern.test(email);
};

export const AddPatientModal = ({ open, setOpen, refresh, setRefresh }: Props) => {
  ...
  const isSmallScreen = useMediaQuery('(max-width:563.33px)');
  ...
  const handleSubmit = async() => {
    ...
    // Error checking for compulsory input data that is not submitted. 
    // Required fields
    const findMissingValues = () => {
      const missingValues = Object.entries(apiData)
        .filter(([key, value]) => {
          if (key === 'address2') return false;
          if (key === 'medicare_no' && !medicareSelected) return false;
          if ((key === 'health_fund_name' || key === 'health_fund_no') && !healthFundSelected) return false;
    ...
    if (missingValues.length > 0) {
      const errorMessage = `Please fill out the following fields: ${missingValues.join(', ')}`;
    ...
    const res = await apiCall('POST', 'patients/create', apiData);
    if (res.error) {
      console.log(res.error);
    } else {
      console.log(`Patient ${res.iid} created successfully`);
      setRefresh(!refresh);
    }
    setPatientData(initData);
    setOpen(false);
